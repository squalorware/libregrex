...
fn freeIteratorCallback(alloc: std.mem.Allocator, value: *FindIterator) void {
    _ = alloc;
    value.deinit();
}

/// Managed opaque handler for C compatibility implementations
pub const ManagedIterator = ManagedOpaqueWrapper(ext.C_IterHolder, FindIterator, freeIteratorCallback);
